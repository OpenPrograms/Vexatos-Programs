...
local capbank = component.isAvailable("capacitor_bank")
...
if capbank then
  capbank = component.capacitor_bank
end
...
if not silent then
  ...
  if capbank then
    print("Running in Capacitor Bank mode.")
  end
...
while true do
  ...
  local maxStored = maxEnergy

  if capbank then
    stored = stored + capbank.getEnergyStored()
    maxStored = maxStored + capbank.getMaxEnergyStored()
    offs = #tostring(maxStored) + 5
  end

  if stored/maxStored <= turnOn and not reactor.getActive() then
    --The reactor is off, but the power is below the turnOn percentage
    reactor.setActive(true)
  elseif stored/maxStored >= turnOff and reactor.getActive() then
